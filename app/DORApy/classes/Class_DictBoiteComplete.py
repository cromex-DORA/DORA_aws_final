...
class DictBoiteComplete(dict):
    ...
    #PARTIE Custom
    ##########################################################################################
    def actualisation_cote_bb_custom(self,dict_info_custom,df_contour_global):
        marge_bord = 5/100
        df_info_custom = pd.DataFrame.from_dict(dict_info_custom, orient="index")
        self = pd.merge(self,df_info_custom[["min_x_custom","max_x_custom","min_y_custom","max_y_custom"]],left_on="CODE_custom",right_index=True)
    ...
    def intersection_ligne_buffer(self,df_buffer_custom,df_info_custom,type_placement):
        ...
        if self.orientation=='normal':
            if len(self.df)>0:
                ...
                geometry_custom_buffer_tempo = df_buffer_custom['geometry_custom_buffer']
    ...
    def placement_des_blocs_interieur_boite_complete(self,dict_dict_info_custom,type_boite_placement:list=['normal']):
        ...
        if self.orientation in type_boite_placement:
            for nom_bloc,dict_bloc in self.items():
                if dict_bloc.type=='bloc_texte_simple':
                    dict_bloc = BlocTexteSimple.placement_bloc_texte_simple_dans_boite(dict_bloc,dict_dict_info_custom)
                if dict_bloc.type=='bloc_icone':
                    dict_bloc = BlocIcone.placement_bloc_icone_dans_boite(dict_bloc)
                if dict_bloc.type=='bloc_lignes_multiples':
                    dict_bloc = BlocLignesMultiples.placement_bloc_lm_dans_boite(dict_bloc,dict_dict_info_custom)
        return self
